...
def parse_rule_colors(rule) -> dict | None:
    ...
    # returns a broken down format of a css block.
...
def process_colors(input_file: str) -> list[dict]:
    """
    Iterates through broken down chunks of css blocks (broken into selector, text, background, and line) and tunes the color pairs (text and background).
    Ignores blocks which has neither text property nor background property or both or if any of them have invalid css colors.
    Returns broken down chunks of CSS blocks with new and tuned colors or the original colors if tuning is not possible or has invalid css color.
    Chunks is simply a python dictionary with keys and values extracted from css.
    """
    processed_colors = []
...
def construct_css_block(css_block: dict) -> str:
    """
    Constructs CSS code with syntax out of chunks generated by `process_colors` function.
    Adds a comment inside each block stating the accessbilty of the tuned or original colors (pairs)
    Can only construct one block out of one broken down chunk.
    """
...
def write_css(processed_colors: list[dict], output_file: str) -> None:
    """
    Leverages `construct_css_block` function to iterate each chunk, construct css code, and write them into output file of desired filename.
    """
