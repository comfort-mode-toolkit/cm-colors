...
class TestColorParser:
    ...
    @given(
        hex_color=st.text(alphabet="0123456789abcdefABCDEF", min_size=6, max_size=6)
    )
    ...
    def test_recursive_parsing_safety(self):
        ...
        result = parse_color_to_rgb("rgba(255, 0, 0, 0.5)", background="rgba(0, 0, 255, 0.5)")
        assert is_valid_rgb(result)
    ...
    def test_boundary_values(self):
        ...
        assert result_transparent == (255, 255, 255)  # should be white (background)
