...
class TestFormatPreservationOnSuccess:
    ...
    def test_hex_format_on_success(self):
        ...
        result, success = pair.tune_colors()
...
class TestFormatPreservationWithDetails:
    """Test format preservation in detailed output."""
    
    def test_hex_in_details_on_success(self):
        """Hex format in detailed output when tuning succeeds."""
        pair = ColorPair("#777", "white")
        result = pair.tune_colors(details=True)
        assert result['status'] is True
        tuned = result['tuned_text']
        assert isinstance(tuned, str)
        assert tuned.startswith("#")
    
    def test_hex_in_details_on_failure(self):
        """Hex format in detailed output even when tuning fails."""
        pair = ColorPair("#cfff04", "white")
        result = pair.tune_colors(details=True)
    ...
    def test_rgb_in_details_on_failure(self):
        """RGB format in detailed output even when tuning fails."""
        pair = ColorPair("rgb(207, 255, 4)", "white")
        result = pair.tune_colors(details=True)
        assert result['status'] is False
        tuned = result['tuned_text']
        assert isinstance(tuned, str)
        assert tuned.startswith("rgb(")
...
class TestSuccessFlagAccuracy:
    ...
    def test_success_true_means_accessible(self):
        ...
        if success:
            ...
            assert new_pair.wcag_level in ["AA", "AAA", "AA Large", "AAA Large"]
            assert new_pair.contrast_ratio >= 4.5  # At least AA for normal text
    ...
    def test_success_false_means_not_accessible(self):
        ...
        if not success:
            ...
            new_pair = ColorPair(result, "white")
            assert new_pair.wcag_level == "FAIL"
            assert new_pair.contrast_ratio < 4.5
...
class TestVisualizerDoesNotAffectReturn:
    ...
    def test_show_flag_preserves_format_on_success(self, capsys):
        ...
        pair = ColorPair("#777", "white")
        result_with_show = pair.tune_colors(show=True)
        pair2 = ColorPair("#777", "white")
        result_without_show = pair2.tune_colors(show=False)
