...
class TestColorWorkflowIntegration:
    ...
    def test_large_text_vs_normal_text_workflow(self):
        ...
        normal_tuned, normal_accessible = self.cm.tune_colors(
            text_rgb, bg_rgb, large_text=False
        )

        # Test large text
        large_wcag = self.cm.wcag_level(text_rgb, bg_rgb, large_text=True)
        large_tuned, large_accessible = self.cm.tune_colors(
            text_rgb, bg_rgb, large_text=True
        )
    ...
    def test_rgb_oklch_lab_roundtrip(self):
        ...
        for rgb in test_colors:
            ...
            for orig, back in zip(rgb, rgb_back):
                assert (
                    abs(orig - back) <= 3
                ), f'Roundtrip failed for {rgb}: got {rgb_back}'
    ...
    def test_accessibility_with_color_similarity(self):
        ...
        for text_rgb, bg_rgb in problematic_colors:
            ...
            if accessible:
                ...
                assert (
                    delta_e < 100
                ), f'Tuned color very different: Delta E = {delta_e}'
...
class TestRealWorldScenarios:
    ...
    def test_web_design_color_palette(self):
        ...
        assert accessibility_results['text_on_background']['wcag'] in [
            'AA',
            'AAA',
        ]
...
def test_quick_integration():
    ...
    same_tuned, same_accessible = cm.tune_colors(
