...
def test_premium_upgrade():
    ...
    res_std, success_std = check_and_fix_contrast(
        text, bg, mode=1, premium=False
    )
    assert success_std
    assert res_std == text

    # Premium mode: should tune it to reach AAA (7.0)
    res_prem, success_prem = check_and_fix_contrast(
        text, bg, mode=1, premium=True
    )
    assert success_prem
...
def test_large_text_handling():
    ...
    res_norm, success_norm = check_and_fix_contrast(
        text, bg, large=False, mode=1
    )
    res_norm_rgb = parse_result(res_norm)
    assert calculate_contrast_ratio(res_norm_rgb, bg_rgb) >= 4.5

    # Large text (needs 3.0)
    res_large, success_large = check_and_fix_contrast(
        text, bg, large=True, mode=1
    )
